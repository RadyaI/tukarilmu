...
export default function UserPublicProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"video" | "post" | "portfolio">("video");
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (!userDoc.exists()) { router.push("/not-found"); return; }
      const userData = { id: userDoc.id, ...userDoc.data() };
      setProfileUser(userData);
      setPortfolioItems((userData as any).portfolioItems || []);
      ...
      setVideos(videosSnap.docs.map(d => ({ id: d.id, ...d.data() })).sort(sortByDate));
      setPosts(postsSnap.docs.map(d => ({ id: d.id, ...d.data() })).sort(sortByDate));
    } catch {
  ...
  const formatPrice = (price: number) =>
    price === 0 ? "Gratis" : new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(price);

  const getChatId = () => {
    if (!currentUser) return null;
    return [currentUser.uid, profileId].sort().join("-");
  };
  ...
  const displayData = activeTab === "video" ? videos : activeTab === "post" ? posts : [];
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        <Link href="/explore" className="inline-flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors font-semibold mb-8 group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 flex flex-col gap-5">
            ...
            >
              ...
              <h1 className="text-xl font-extrabold text-slate-900 mb-1">{profileUser.name || "Pengguna TukarIlmu"}</h1>
              <p className="text-sm text-slate-500 mb-3">{profileUser.email}</p>

              <span className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold border mb-4 ${tagStyle.bg} ${tagStyle.text} ${tagStyle.border}`}>
                <TagIcon className="w-3.5 h-3.5" /> {tag}
              </span>

              {profileUser.bio && (
                <p className="text-sm text-slate-600 leading-relaxed mb-4 px-2 italic">"{profileUser.bio}"</p>
              )}

              {!isOwnProfile && (
                currentUser ? (
                  <Link href={`/chats/${chatId}`} className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl transition-all shadow-md hover:shadow-indigo-200 cursor-pointer">
                    <MessageCircle className="w-5 h-5" /> Kirim Pesan
                  </Link>
                ) : (
                  <Link href="/login" className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-slate-100 hover:bg-indigo-50 text-indigo-600 font-bold rounded-2xl transition-all border border-slate-200 cursor-pointer">
                    <MessageCircle className="w-5 h-5" /> Login untuk Chat
                  </Link>
                )
              )}
              {isOwnProfile && (
                <Link href="/dashboard/profile" className="w-full text-center px-5 py-3 bg-slate-50 hover:bg-indigo-50 text-slate-700 font-bold rounded-2xl transition-all border border-slate-200 text-sm cursor-pointer">
                  Edit Profil Kamu
                </Link>
              )}
            </motion.div>
            ...
            {(profileUser.igUrl || profileUser.linkedinUrl || profileUser.portfolioUrl) && (
              ...
              >
                ...
                <div className="flex flex-col gap-3">
                  {profileUser.igUrl && (
                    <a href={profileUser.igUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm font-semibold text-slate-600 hover:text-pink-500 transition-colors group">
                  ...
                  {profileUser.linkedinUrl && (
                    <a href={profileUser.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm font-semibold text-slate-600 hover:text-blue-600 transition-colors group">
                  ...
                  {profileUser.portfolioUrl && (
                    <a href={profileUser.portfolioUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm font-semibold text-slate-600 hover:text-emerald-600 transition-colors group">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            >
              <h2 className="text-lg font-extrabold text-slate-900 mb-6">
                Konten dari {profileUser.name?.split(" ")[0] || "Kreator"}
              </h2>
              ...
              <div className="flex p-1 bg-slate-100 rounded-2xl mb-6">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${activeTab === "video" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
                >
                  <PlayCircle className="w-3.5 h-3.5" /> Video ({videos.length})
                </button>
                <button
                  onClick={() => setActiveTab("post")}
                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${activeTab === "post" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500"}`}
                >
                  <FileText className="w-3.5 h-3.5" /> Artikel ({posts.length})
                </button>
                <button
                  onClick={() => setActiveTab("portfolio")}
                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${activeTab === "portfolio" ? "bg-white text-fuchsia-600 shadow-sm" : "text-slate-500"}`}
                >
                  <Briefcase className="w-3.5 h-3.5" /> Portfolio ({portfolioItems.length})
                </button>
              </div>
              ...
              {activeTab === "portfolio" ? (
                portfolioItems.length === 0 ? (
                  <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl">
                    ...
                    <p className="text-slate-400 font-semibold text-sm">Belum ada portfolio yang ditampilkan.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {portfolioItems.map((item, i) => (
                      <motion.div
                        ...
                        className="group bg-white rounded-[1.5rem] border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all overflow-hidden"
                      >
                        {/* Thumbnail */}
                        <div className="w-full h-36 bg-slate-100 flex items-center justify-center relative overflow-hidden">
                          {item.fileType === "image" && item.fileUrl ? (
                            <>
                              <img src={item.fileUrl} alt={item.title} className="w-full h-full object-cover absolute inset-0 group-hover:scale-105 transition-transform duration-500" />
                          ...
                          <div className="absolute top-2 right-2 flex gap-1.5 z-10">
                            {item.fileUrl && (
                              <a
                                ...
                                className="bg-white/90 backdrop-blur-sm px-2.5 py-1.5 rounded-full text-[10px] font-bold text-slate-700 flex items-center gap-1 shadow-sm hover:bg-indigo-50 hover:text-indigo-600 transition-colors"
                                title={item.fileType === "pdf" ? "Buka PDF" : "Buka Gambar"}
                              >
                                <ExternalLink className="w-3 h-3" />
                                {item.fileType === "pdf" ? "PDF" : "Lihat"}
                              </a>
                            )}
                            {item.link && (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="bg-white/90 backdrop-blur-sm px-2.5 py-1.5 rounded-full text-[10px] font-bold text-slate-700 flex items-center gap-1 shadow-sm hover:bg-emerald-50 hover:text-emerald-600 transition-colors"
                                title="Buka Link"
                              >
                        ...
                        {/* Info */}
                        <div className="p-4">
                          <h3 className="font-bold text-slate-900 text-sm line-clamp-1 mb-1">{item.title}</h3>
                          {item.description && (
                            <p className="text-xs text-slate-500 text-justify leading-relaxed">{item.description}</p>
                          )}
                          ...
                          <div className={`w-full h-36 flex items-center justify-center relative overflow-hidden ${activeTab === "video" ? "bg-indigo-50" : "bg-fuchsia-50"}`}>
                            {item.thumbnailUrl ? (
                              <>
                                ...
                                {activeTab === "video" ? <PlayCircle className="w-9 h-9 text-white/90 z-10 drop-shadow" /> : <FileText className="w-9 h-9 text-white/90 z-10 drop-shadow" />}
                              </>
                            ) : (
                              activeTab === "video" ? <PlayCircle className="w-9 h-9 text-indigo-300" /> : <FileText className="w-9 h-9 text-fuchsia-300" />
                            )}
